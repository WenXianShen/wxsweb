<template>
  <div style="padding: 10px" id="menuAdd">
    <div
      ...
      <div style="height: 100px;width: 100%;">
        <Form :model="formItem" :label-width="100" :rules="ruleValidate"  ref="formItem">
          ...
          <FormItem
            label="父级菜单："
            style="width: 30%;"
            v-if="parentIsShow" prop="parentId"
          >
          ...
          <FormItem
            label="路由名称："
            style="width: 30%;" prop="routerPath"
          >
            <Input v-model="formItem.routerPath"></Input>
          </FormItem>
          <FormItem
            label="排序："
            style="width: 30%;" prop="displaySeq"
          >
            <Input v-model="formItem.displaySeq"></Input>
          </FormItem>
          <FormItem
            label="是否展示："
            style="width: 30%;" prop="isHidden"
          >
            <i-select
              v-model="formItem.isHidden"
            >
          ...
          <Form-item>
            ...
            <Button style="margin-left: 20px;"  onclick="javascript:window.history.back(-1);">取消</Button>
          </Form-item>
        </Form>
      </div>
    </div>
  ...
  data () {
    return {
      ...
      ruleValidate: {
        ...
        lvl: [
          {required: true, message: '请选择菜单级别', trigger: 'change', type: 'number'}
        ],
        parentId: [
          {required: true, message: '请选择父级菜单', trigger: 'change', type: 'number'}
        ],
        routerPath: [
          {required: true, message: '请输入路由', trigger: 'blur'}
        ],
        isHidden: [
          {required: true, message: '请选择是否展示', trigger: 'change', type: 'number'}
        ]
      },
      menuList: []
    }
  ...
  methods: {
    ...
    handleSubmit (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          let that = this
          this._UTIL.jpost(this._API.menu.saveMenu, this.formItem, function (data) {
